	...
	public void readInput (String input) {
		...
		for (int i = 0; i < allComms.size(); i++)
		{
			...
			if (nextCommand.equals("fd"))	// If it's fd, move forward a certain number of pixels
			{
				...
				pen.setNewOffsetX(pen.getOffsetX() + Double.parseDouble(allComms.get(i+1)) * Math.sin(Math.toRadians(pen.getOrientation())));
				pen.setNewOffsetY(pen.getOffsetY() + Double.parseDouble(allComms.get(i+1)) * Math.cos(Math.toRadians(pen.getOrientation())));
				i++;	// Skip the next command (the number of pixels to go forward)
			...
			{
				// Find the new offsets of the pen using a little trig
				pen.setNewOffsetX(pen.getOffsetX() + Double.parseDouble(allComms.get(i+1)) * Math.sin(Math.toRadians(pen.getOrientation() + 180)));
				pen.setNewOffsetY(pen.getOffsetY() + Double.parseDouble(allComms.get(i+1)) * Math.cos(Math.toRadians(pen.getOrientation() + 180)));
				i++;
			...
			{
				pen.setOrientation((pen.getOrientation() + Double.parseDouble(allComms.get(i+1))) % 360);
